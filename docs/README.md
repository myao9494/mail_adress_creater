...
ブラウザからのAPIリクエストは同一Originのみ許可し、別OriginからのPOST/PUT/OPTIONSは拒否します。
...
名前にカンマを含む場合はダブルクォートで囲めます。回数が整数として読めない行は読み込み対象外です。フロントエンドはCSVを`no-store`で取得し、取得失敗時はエラーを表示します。
