...
`POST /api/add-schedule`はOutlook COMで予定表アイテムを作成する。日時だけでなく「明日」「今週の水曜日」などの相対表現を解析する。部屋番号など時刻ではない数字は時刻として扱わない。本文にHTMLタグが含まれる場合は`appointment.HTMLBody`として保存し、失敗時はプレーンテキストにフォールバックする。
