...
- よく使う宛先グループを保存するお気に入り
- 自然文の予定解析とOutlook予定表への追加
- SQLiteの内容確認と一部テーブルの選択削除
...
- 1行目はヘッダー（`名前,回数`）
- 名前：Outlookに貼り付ける表示名
- 回数：送信回数（並び替えに使用）。整数として読めない行は読み込み対象外
- 名前にカンマを含む場合はCSVの標準形式どおりダブルクォートで囲めます
...
- 「予定解析」/「Outlook追加」: 自然文の予定入力から件名、日時、場所、終日予定を抽出し、必要に応じて編集してOutlook予定表へ追加します。
- ハンバーガーメニュー: キーワード、アドレス再取得の自動実行頻度、キーワードチェックの自動実行頻度、お気に入り、DB表示、ダミーデータ投入を操作できます。
- お気に入り: 複数の宛先を名前付きで保存できます。検索結果では`★`付きで表示され、コピー時は登録済み宛先へ展開されます。
- DB表示: `keyword_matches`、`recipients`、`favorites`、`job_runs`、`settings`を閲覧できます。`settings`以外は選択削除でき、`recipients`削除時はCSVも同期更新します。
...
- APIは同一Originからのブラウザリクエストのみ許可します。別OriginからのPOST/PUT/OPTIONSは拒否され、別Originに対する`Access-Control-Allow-Origin`は返しません。
