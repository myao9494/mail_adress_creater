...
6. **職アドチェック**: 登録キーワードに一致する通知行をSQLiteへ保存し、未確認の一致を画面に強調表示。ユーザーが「OK」ボタンを押すまで通知が消えない（アプリ起動時も未確認通知を自動復元）。`keyword_matches` テーブルに `confirmed` カラムを持ち、`POST /api/keyword-matches/confirm` で確認済みにする。
